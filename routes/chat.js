...
我先把本轮信息收束到这里。建议你根据上面的回复核对项目要求、整理投递材料，再决定是否提交申请。`;
...
function buildSystemPrompt(job) {
  ...
  return `你是 about编辑部招募助手。用户正在咨询about编辑部的项目相关问题，请保持简洁、友好，用中文回答。

当前咨询的项目：「${job.title}」
项目描述：${job.description || '暂无'}
具体要求：
- ${reqs || '暂无'}
...
规则：
1. 只回答与本项目和about编辑部投递流程相关的问题。
2. 能从项目信息直接回答的问题，直接给出简洁答案，不要升级人工。
3. 不要编造项目信息中没有的内容。
4. 如果用户询问“我是否适合/作品够不够/能不能投”，先用项目要求给出自查清单和建议，不要直接升级人工。
5. 如果项目信息没有明确写，先说明“当前项目信息里没有明确写”，再给出基于已知信息的投递建议；不要因为信息缺失就升级。
...
function withLimitNotice(reply) {
  const notice = `本轮咨询已达到 ${AI_USER_MESSAGE_LIMIT} 条，我先帮你把能确认的信息收束在这里。建议你根据上面的回复核对项目要求、整理投递材料。`;
  if (!reply) return notice;
  return `${reply}\n\n${notice}`;
}
